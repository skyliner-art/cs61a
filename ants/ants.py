...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        def check(place,distance):
            if place.is_hive is True or distance>self.upper_bound:
                return None
            elif random_bee(place.bees) is not None and distance >= self.lower_bound:
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        ShortThrower.action(self,gamestate)
        next_place = self.place.exit
        while next_place is not None:
            if next_place.ant is not None:
                next_place.ant.double()
                if next_place.ant.is_container is True and next_place.ant.ant_contained is not None:
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        while now_place is not None:
            if now_place.ant is not None and now_place.ant is not self:
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem EC 4
        real_damage =max(self.damage - distance*0.25 - self.insects_shot*0.0625,0)
        return real_damage
        # END Problem EC 4

    def action(self, gamestate):
        insects_and_distances = self.insects_in_front()
        for insect, distance in insects_and_distances.copy().items():
